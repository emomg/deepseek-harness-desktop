...
- 🔄 复用已有服务：如果 3080 端口已有 dsh 实例，直接连接，不会重复启动；退出应用保留 dsh 服务，下次打开秒连
...
- 🐋 **黑鲸鱼主题**：冷启动黑鲸鱼动画（≤1s）+ 主面板液态磨砂玻璃背景（默认开启，可关闭）
- ⚡ 快速冷启动：窗口先显示启动占位页（黑鲸鱼 + 已等待计时），dsh 后台线程拉起；就绪探测用真实 HTTP 探活
...
| 🎨 **皮肤中心** | **11 款原创皮肤**（黑鲸 / 深空 / 液态玻璃 / 极光 / 赛博 / 极夜 / 墨玉 / 晨曦 / 薄荷 / 樱花 / 纯白），全部**先试穿再应用**：试穿即时生效、退出/离开完全还原，满意一键应用；原「个性化」微调（可见度 / 模糊 / 面板透明度 / 液态动画 / 背景样式）已并入皮肤中心 |
| ⚙️ **设置管理页** | 设置左侧新增：**技能管理**（含 `$DSH_HOME\skills` 用户技能库与预设技能库）、**MCP 服务器管理**（状态 / 传输 / 工具数） |
