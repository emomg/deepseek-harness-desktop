...
| ✅ **评审门禁** | 对工作区改动**逐文件审阅**：git 仓库以 HEAD 为基线，非 git 用复制基线；每文件查看 diff → 接受/拒绝 → 提交已接受（git commit）/ 放弃（全部恢复） | Pro 面板「评审」页 |
...
│   │   review(start·list·diff·accept·reject·commit·discard)
...
| review.js | 评审门禁（基线/刷新/差异/接受/拒绝/提交/放弃；git 可注入） |
...
- review.test.js：git 基线（fake git 注入）+ 非 git 复制基线（真实 fs）全流程
...
- 同一工作区同时只允许一个进行中的评审
- 非 git 工作区的差异用纯 JS 行 diff（LCS，大文件退化为全文对照）
