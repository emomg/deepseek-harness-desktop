...
桌面端每次启动会在后台**静默检查一次更新**（数据源：本仓库 GitHub Releases），
发现新版本时弹出提示并询问：

- **「是」** → 自动下载对应版本变体的安装包（精简版只下载精简包、full 版只下载
  `-full` 包、专业版只下载 `-pro` 包，绝不跨变体推荐），校验 SHA256 后静默安装，
  完成后自动启动新版；
- **「否」** → 继续使用当前版本，下次启动再提醒。

也可随时从系统托盘菜单 →「检查更新」手动触发。更新检查基于 `edition.txt`
（安装包写入的变体标记）+ 本仓库最新 Release 的 semver 比较；网络不可用时静默跳过，
不影响使用。设置环境变量 `DSH_PRO_DISABLE_UPDATE=1` 可关闭自动检查。
