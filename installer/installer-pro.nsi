; DeepSeek Harness Desktop - Windows 安装脚本（NSIS 3，专业版）
Unicode true
!include "MUI2.nsh"
...
Name "${APP_NAME}"
!ifndef SETUP_NAME
  !define SETUP_NAME "Setup-${APP_VERSION}.exe"
!endif
OutFile "${SETUP_NAME}"
InstallDir "${INST_DIR}"
...
VIAddVersionKey /LANG=2052 "FileDescription" "DeepSeek Harness 桌面客户端安装程序（专业版）"
...
Section "主程序" SecMain
  ...
  ; 版本变体标记：桌面端更新检查按它选择对应安装包（lite / full / pro）
  ...
  !endif
  SetOutPath "$INSTDIR"
  ; WebView2 Runtime 检查：缺失则用捆绑的引导器静默安装（引导器缺失时跳过）
  !if /FileExists "build\MicrosoftEdgewebview2Setup.exe"
